...
router.get('/', requireAuth(), async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM events WHERE squad_id = $1 ORDER BY event_date, event_time', [squadId]);
...
router.post('/', requireAuth(), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO events (squad_id, type, title, event_date, event_time, location) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
...
router.patch('/:id', requireAuth(), async (req, res) => {
  try {
    ...
    const { type, title, event_date, event_time, location } = req.body;
    const result = await pool.query(
      `UPDATE events SET type=COALESCE($1,type), title=COALESCE($2,title), event_date=COALESCE($3,event_date), event_time=COALESCE($4,event_time), location=COALESCE($5,location), updated_at=now() WHERE id=$6 RETURNING *`,
